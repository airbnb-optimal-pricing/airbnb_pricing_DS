...
with open('./simp_model_v1.pk','rb') as f:
  simp_model = pickle.load(f)
with open('./simp_cols.pk','rb') as f:
  simp_cols = pickle.load(f)
with open('./simp_num_preprocessor.pk','rb') as f:
  simp_num_preprocessor = pickle.load(f)
with open('./simp_cat_preprocessor.pk','rb') as f:
  simp_cat_preprocessor = pickle.load(f)
...
def get_simp_prediction(zipcode, bedrooms, bathrooms):
    ...
    # Use train data preprocessor
    cat_transformed = simp_cat_preprocessor.fit_transform(df_cat)
    
    # Use train data preprocessor
    num_transformed = simp_num_preprocessor.fit_transform(df_num)
    num_transformed = pd.DataFrame(num_transformed, columns=num_cols)
    
    # Concatenate numeric and categorical dataframes
    df_transformed = pd.concat((num_transformed, cat_transformed), axis=1)
    # Create blank dataframe using columns from transformed train data
    df_blank = pd.DataFrame(columns=simp_cols)
    
    # Concatenate  
    df_model = pd.concat((df_blank, df_transformed))
    df_model = df_model.replace(np.nan, 0)
    
    y_pred = simp_model.predict(df_model)
    simp_prediction = int(math.exp(y_pred[0]))

    return simp_prediction